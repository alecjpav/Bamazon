	...
	prompt.get(['number'], function(err, result) {
		if(result.number.charCodeAt() >= 48 && result.number.charCodeAt() <= 57) {
			if(result.number > 0 && result.number <= item.StockQuantity) {
				...
				console.log("Thank you for your order. Your total amount will be: $" + (result.number * item.Price) + '\n');
				...
				console.log("Sorry, there isn't enough quantity to fulfill this order.");
